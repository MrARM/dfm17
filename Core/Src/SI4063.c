...
// Utils
void SPI_assert_SS(){
	HAL_GPIO_WritePin(pRADIO_CS_GPIO_Port, pRADIO_CS_Pin, GPIO_PIN_RESET);
}

void SPI_deassert_SS(){
	HAL_GPIO_WritePin(pRADIO_CS_GPIO_Port, pRADIO_CS_Pin, GPIO_PIN_SET);
}
...
void SI4063_init(uint32_t ref_freq, uint8_t tcxo, SPI_HandleTypeDef *hspi)
{
	...
	SPI_deassert_SS();
	HAL_Delay(10);
	SI4063_rx_CTS(rx_buffer, 1);

	/* Assumes Timepulse to Xin pin already enabled. */
	// TODO: Figure out what this does. Might be important or may not be necessary.
//	if(tcxo)
//	{
...
//	}
	...
	HAL_GPIO_WritePin(pRADIO_GPIO2_GPIO_Port, pRADIO_GPIO2_Pin, GPIO_PIN_RESET);		// set PB0 (GPIO2 - TXDATA) LOW


	/* POWER_UP Sequence */
	HAL_GPIO_WritePin(pRADIO_SDN_GPIO_Port, pRADIO_SDN_Pin, GPIO_PIN_SET);
	HAL_Delay(1);
	HAL_GPIO_WritePin(pRADIO_SDN_GPIO_Port, pRADIO_SDN_Pin, GPIO_PIN_RESET);				// set SDN LOW
	HAL_Delay(10);														// wait for POR to finish
	...
	SPI_deassert_SS();
	SI4063_rx_CTS(rx_buffer, 9);

	/* Si4x6x Pins Setup */
	SI4063_setup_pins(8, 16, 4, 1, 0, 0);
}
...
{
	SI4063_change_state(1);												// SLEEP/STANDBY
...
/*
 * UNUSED!! There's no trace connected to GPIO0
	CLEAR TO SEND (CTS)
		Checking CTS is used to make sure the internal MCU has executed the command and prepared the data to be output over the SDO pin.
...
{
	SI4063_set_modulation(0, 1);										// CW, Asynchronous
	SI4063_set_frequency(frequency, ref_freq);
	SI4063_set_frequency_offset(modem_freq_offset);
...
*/
//void SI4063_tx_FSK_rtty(uint8_t * packet, uint8_t length, uint16_t baud, uint32_t frequency,
//						uint32_t ref_freq, uint16_t modem_freq_dev, uint16_t modem_freq_offset)
//{
//	set_modulation(2, 1);										// 2FSK, Asynchronous
//	set_frequency(frequency, ref_freq);
//	set_frequency_deviation(modem_freq_dev);
//	set_frequency_offset(modem_freq_offset);
//
...
//		{
//			if(c & 0x01) HAL_GPIO_WritePin(pRADIO_GPIO2_GPIO_Port, pRADIO_GPIO2_Pin, GPIO_PIN_SET);		// set PB0 (GPIO2 - TXDATA) HIGH
//			else HAL_GPIO_WritePin(pRADIO_GPIO2_GPIO_Port, pRADIO_GPIO2_Pin, GPIO_PIN_RESET);		// set PB0 (GPIO2 - TXDATA) LOW
//
//			c >>= 1;
//
//			while(!TC1_compare_match());
...
//
//		/* Stop Bits */
//		HAL_GPIO_WritePin(pRADIO_GPIO2_GPIO_Port, pRADIO_GPIO2_Pin, GPIO_PIN_SET);		// set PB0 (GPIO2 - TXDATA) HIGH
//		while(!TC1_compare_match());
//		HAL_GPIO_WritePin(pRADIO_GPIO2_GPIO_Port, pRADIO_GPIO2_Pin, GPIO_PIN_SET);		// set PB0 (GPIO2 - TXDATA) HIGH
//		while(!TC1_compare_match());
//
...
//*/
//void SI4063_tx_GFSK_aprs(uint8_t * bitstream, uint16_t length, uint32_t frequency, uint32_t ref_freq, uint16_t modem_freq_offset)
//{
...
//
//	set_modulation(3, 0);										// 2GFSK, Synchronous
//	set_frequency(frequency, ref_freq);
//	set_frequency_offset(modem_freq_offset);
//	set_frequency_deviation(modem_freq_dev);
//	set_data_rate(modem_data_rate, 1);							// TXOSR: 0 - 10x, 2 - 20x, 1 - 40x
